        ...
        for (int group_idx = start_group_offset; group_idx < total_groups_per_row; group_idx += gridDim.x * groups_per_block) {
            ...
            switch (q_type) {
                case QuantType::FP8: {
                    ...
                    #pragma unroll
                    for (int i = 0; i < ELEMENTS_PER_THREAD; ++i) {
                        float val = (input_float[i] * r_scale);
                ...
                case QuantType::FP4: {
                    ...
                    for (int i = 0; i < ELEMENTS_PER_THREAD / 2; ++i) {
                        float val1 = (input_float[i*2] * r_scale);
                        val1 = fminf(fmaxf(val1, -FP4_MAX), FP4_MAX);
                        float val2 = (input_float[i*2+1] * r_scale);
...
template <
    ...
    typename F4ScaleTensor, typename F6ScaleTensor, typename F8ScaleTensor
>
__global__ void downproj_quantize_kernel_with_cute_layout_w4(
    const bf16_t *W,
    uint8_t *f4out, uint8_t *f6out, uint8_t *f8out,
    F4ScaleTensor f4scale, F6ScaleTensor f6scale, F8ScaleTensor f8scale, // 接收 CUTE Tensor 对象
    const int KN, const int KS, const int KO,
    ...
    for (int row_id = blockIdx.y; row_id < total_rows; row_id += gridDim.y) {
        for (int group_idx = start_group_offset; group_idx < total_groups_per_row; group_idx += gridDim.x * groups_per_block) {
            ...
            if (group_idx < KN_GROUPS) {
                ...
                q_type = QuantType::FP6; quant_max_bound = FP4_MAX; group_idx_in_type = group_idx - KN_GROUPS;
                data_out_ptr = f6out + row_id * (KS / 2) + group_idx_in_type * (GROUP_SIZE / 2);
            } else {
                q_type = QuantType::FP8; quant_max_bound = FP4_MAX; group_idx_in_type = group_idx - (KN_GROUPS + KS_GROUPS);
                data_out_ptr = f8out + row_id * (KO / 2) + group_idx_in_type * (GROUP_SIZE / 2);
            }
...
void run_downproj_bf16_mxfp4(
  ...
  int KN, int KS, int KO
    ...
    // --- 启动内核，传入 Tensor 对象 ---
    downproj_quantize_kernel_with_cute_layout_w4<GROUP_SIZE, ELEMENTS_PER_THREAD><<<grids, blocks>>>(
        d_W,
        d_o_normal, d_o_sensitive, d_o_outlier,
        f4scale_tensor, f6scale_tensor, f8scale_tensor, // 传递 Tensor 对象
        KN, KS, KO, out_features
