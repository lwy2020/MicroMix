...
__global__ void rmsnorm_bf16_mixed_kernel(
  bf16_t *input,
  bf16_t *weight,
  float eps,
...
#pragma unroll
  ...
  for(int i = 0;i < elements_per_thread;++i){
    ...
    input_frag[i] = converterScale((float)input_value * (float)weight_value * (float)sumv);
  }
...
void run_rmsnorm_bf16_mixed(
  ...
  rmsnorm_bf16_mixed_kernel<hidden_dim / 32, group_size, hidden_dim><<<grids, blocks>>>(
    (bf16_t *)hidden_states,
    (bf16_t *)weight,
    eps,
    (int16_t *)reorder_index,
...
template void run_rmsnorm_bf16_mixed<32, 4096>(
