...
void run_benchmark(
    ...
    // initialize_matrix(A, M * K, -1.f);
    // initialize_matrix(B, N * K, 2.f);
    // initialize_matrix(C, M * N, 0.f);
    // initialize_matrix(scaleA, szA, 1.f);
    // initialize_matrix(scaleB, szB, 1.f);
    ...
    cudaMalloc(&D_d, 16*M * N * sizeof(ElementD));
    cudaMalloc(&D_d_ref, 16*M * N * sizeof(ElementD));
...
void launch_and_run_benchmark(int M, int N, int K, int timed_iters, bool do_validation, RefKernel ref_kernel) {
    ...
    auto kernel_launcher =
        [=](typename EA::DataType* pA, typename EA::ScaleFactorType* pSFA,
            ...
            ElementD* pD, 
            int m, int n, int k) {
        gemm_host_tn_cpasync<N_STAGE, BM, BN, BK>(pA, pSFA, pB, pSFB, pC, pD, m, n, k);
    };
