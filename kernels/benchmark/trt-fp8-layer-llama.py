...
def build_decoder_layer_engine():
    """构建使用 FP8 的 Llama 3 解码器层 TensorRT 引擎"""
    ...
    def add_fp8_linear_op(input_tensor, weight_tensor, op_type, equation=None, transpose_b=False, op_name=""):
        """
        这个函数通过构建一个 Q-Dq-MatMul 模式来触发 TensorRT 的 FP8 算子融合。
    ...
    q_proj = add_fp8_linear_op(normed_input, q_proj_w, 'einsum', "bsk,kn->bsn", op_name="Q_Proj_Einsum")
    k_proj = add_fp8_linear_op(normed_input, k_proj_w, 'einsum', "bsk,kn->bsn", op_name="K_Proj_Einsum")
    v_proj = add_fp8_linear_op(normed_input, v_proj_w, 'einsum', "bsk,kn->bsn", op_name="V_Proj_Einsum")
    ...
    attention_output = add_fp8_linear_op(attn_out_bsn, o_proj_w, 'einsum', "bsk,kn->bsn", op_name="O_Proj_Einsum")
    ...
    gate_proj = add_fp8_linear_op(normed_residual1, ffn_gate_w, 'einsum', "bsk,kn->bsn", op_name="FFN_Gate_Einsum")
    up_proj = add_fp8_linear_op(normed_residual1, ffn_up_w, 'einsum', "bsk,kn->bsn", op_name="FFN_Up_Einsum")
    
    # 1. 计算 sigmoid(gate_proj)
    sigmoid_gate = network.add_activation(gate_proj, trt.ActivationType.SIGMOID).get_output(0)

    # 2. 计算 SiLU(gate_proj) = gate_proj * sigmoid(gate_proj)  <-- 这是新增的步骤
    ...
    ffn_output = add_fp8_linear_op(gated_ffn, ffn_down_w, 'einsum', "bsk,kn->bsn", op_name="FFN_Down_Einsum")
