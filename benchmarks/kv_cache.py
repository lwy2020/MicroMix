...
def matmul_hadmixedgemm(X, dtype):
    # n = X.shape[-1]
...
def batch_decode_i4(o, q, kv_data, kv_param,
    ...
    return mixedgemm.batch_decode_i4(
...
def append_kv_f16(kv_data, kv_param,
               ...
               v, k_param, v_param,
               layer_idx):
    return mixedgemm.append_kv_f16(
        kv_data, kv_param,
        kv_indptr, kv_indices,
        last_page_offset, k,
        v, k_param, v_param,
...
def batch_decode_f16(o, q, kv_data, kv_param,
               kv_indptr, kv_indices,
               last_page_offset, layer_idx):
    return mixedgemm.batch_decode_f16(
        o, q, kv_data, kv_param,
        kv_indptr, kv_indices,
        last_page_offset, layer_idx)
...
class _AttentionStub(object):
    ...
    def forward(self, q, num_kv_heads, attention_kwargs, layer_idx):
        ...
        if self.hadamard_dtype is not None:
            q = matmul_hadmixedgemm(q, dtype=self.hadamard_dtype) 
...
class MultiLayerPagedKVCache4Bit(Cache):
    ...
    def update(
        ...
        if self.hadamard_dtype is not None:
            key_states = matmul_hadmixedgemm(key_states, dtype=self.hadamard_dtype)
